...
#' 
#' @export
test_query_pairs <- function(gene, marker_pairs, covar_names = NULL, max_pair_cor = NULL, 
  ...
  all_pairs <- marker_pairs
  all_pair_names <- cbind(colnames(gene)[marker_pairs[,1]], colnames(gene)[marker_pairs[,2]])
  ...
  if (run_parallel) {
    ...
    for (p in index) {
        one_check <- check_multi_pairs(pair_list[[p]])
        good_pair_list <- c(good_pair_list, one_check)
    }
  }
