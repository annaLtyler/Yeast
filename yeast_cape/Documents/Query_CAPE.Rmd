...
boot.var <- function(v, nboot = 100){
  all.var <- rep(NA, nboot)
  for(b in 1:nboot){
    all.var[b] <- var(sample(v, replace = TRUE), na.rm = TRUE)
  }
  return(all.var)
}
...
for(ph in 1:ncol(pheno_obj$pheno)){
  ...
  overall.var <- var(log10(pheno_obj$pheno[,ph]), na.rm = TRUE)
  for(i in 1:dim(geno_obj)[3]){
    report.progress(i, dim(geno_obj)[3])
    allele.pheno <- apply(geno_obj[,,i], 2, function(x) pheno_obj$pheno[which(x == 1),ph])  
    one.var <- sapply(allele.pheno, function(x) var(x, na.rm = TRUE))
    allele.var[,i] <- one.var
    #var.order <- order(one.var)
    #boxplot(allele.pheno[var.order], col = categorical_pal(8)[var.order])
  ...
  for(ch in 1:16){
    ...
    plot.new()
    plot.window(xlim = c(1, max(chr.pos)/1e3), ylim = c(min(allele.var), max(allele.var)))
    for(a in 1:8){
