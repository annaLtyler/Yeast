...
if(!file.exists(trait.deviation.file)){
  ...
  trait_deviation <- readRDS(trait.deviation.file)
}
...
trait.cor <- t(sapply(trait_deviation, function(y) sapply(y, function(x) cor(x[,1], x[,2], use = "pairwise.complete.obs"))))
barplot(trait.cor, beside = TRUE);abline(h = 0)
```
...
par(mfrow = c(5,2))
plot_one_variant_effect(final_obj, geno_obj, test.marker)
