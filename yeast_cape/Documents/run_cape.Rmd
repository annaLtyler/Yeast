...
if(subsample.markers){
  ...
  for(tr in 1:length(traits)){
    ...
    all.region.markers <- sapply(1:length(region.chrs), 
      function(x) get_region_idx(region.chrs[x], left.lod.pos[x], right.lod.pos[x], 
      marker.chr, marker.pos))
  ...
  unique.markers <- unique(unlist(unique.marker.idx))
...
First we look at motifs in this network. 
