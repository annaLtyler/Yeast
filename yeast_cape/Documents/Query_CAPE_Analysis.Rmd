...
```{r get_exp_name}
library(here)
exp.names <- c("GAL_query", "GAL4_query")
...
```{r overlapping_interactions}

all.nets <- vector(mode = "list", length = length(exp.names))
names(all.nets) <- exp.names

for(i in 1:length(var_inf)){
  int.net <- graph_from_edgelist(var_inf[[i]][,c("Source", "Target")])
  V(int.net)$chrom <- sapply(V(int.net)$name, function(x) substr(x, 2,3))
  E(int.net)$effect <- as.numeric(var_inf[[i]][,"Effect"])
  E(int.net)$p <- as.numeric(var_inf[[i]][,"p_adjusted"])
  all.nets[[i]] <- int.net
}
