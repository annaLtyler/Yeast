...
```{r effect_sizes}
#head(var_inf)
query.as.source <- which(var_inf[,"Source"] == "query")
query.as.target <- which(var_inf[,"Target"] == "query")
source.effects <- as.numeric(var_inf[query.as.source,"Effect"])
target.effects <- as.numeric(var_inf[query.as.target,"Effect"])

boxplot(list(source.effects,target.effects), 
  names = c("Query as Source", "Query as Target"))
abline(h = 0)
```

```{r pheno_effects, fig.width = 8, fig.height = 8}
pheno.effects <- pheno.effects.DO(final_obj, geno_obj, covar = NULL, 
