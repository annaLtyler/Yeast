...
scan1.results <- scan1(genoprobs, sub.traits[,traits], kinship = K)
...
```{r allele_effects, eval = FALSE}
#out of Sash3 curiosity I'd like to look at the allele effects on the X chromosome
weight.coef <- scan1coef(genoprobs[,"X"], sub.traits[,traits[1]], kinship = K[[20]])
plot_coefCC(weight.coef, map = map)
ins.coef <- scan1coef(genoprobs[,"X"], sub.traits[,traits[2]], kinship = K[[20]])
plot_coefCC(ins.coef, map = map)
```
