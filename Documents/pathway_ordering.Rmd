...
```{r load_cape, message = FALSE, error = FALSE, warning = FALSE}
#load local version of cape code
source("~/Documents/git_repositories/useful_r_code/load_libraries.R")
needed.libraries <- c("qtl2", "abind", "Matrix", "MASS", "regress", "igraph",
"RColorBrewer", "rmarkdown", "pheatmap", "knitr")
load_libraries(needed.libraries)

library(R6);library(yaml);library(abind);library(tools);library(caTools)
library(RColorBrewer);library(Matrix);library(propagate);library(igraph)
library(qtl);library(qtl2);library(regress);library(evd);library(qtl2convert)
cape.fun <- list.files("~/Documents/git_repositories/cape/R", pattern = ".R", full.names = TRUE)
